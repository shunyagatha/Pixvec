...
| `--underpaint` / `--no-underpaint` | Paint the opaque silhouette once underneath everything, as a single path, so an interior seam cannot be see-through. Two abutting fills are two elements, and at their shared edge the pair leaves the pixel up to a quarter transparent; with no background rectangle to hide that — which is to say, on artwork with transparency — the gap traces every interior boundary. **On in `--preset clean`**, where it takes logo-tux from 3,621 leaking pixels (3,171 of them nowhere near the silhouette) to 319 and **zero**, and alpha-dice from 1,350/725 to 243/16, beating the paid rival on both. Costs 4% gzip on logo-tux and 12% on alpha-dice, adds no coordinate that was not already in the file, and is a no-op — byte-identical output — on artwork with no transparent pixels |
