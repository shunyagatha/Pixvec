...
async function runVectorize(input: string, o: VectorizeCliOptions): Promise<void> {
  ...
  if (looksLikeSvg(bytes)) {
    fail(`${input} is already an SVG. Did you mean \`graver rasterize\`?`);
  }
...
async function runRasterize(input: string, o: RasterizeCliOptions): Promise<void> {
  const bytes = await readInput(input);
  if (!looksLikeSvg(bytes)) {
    fail(`${input} does not look like an SVG. Did you mean \`graver vectorize\`?`);
  }
...
async function runEdit(input: string, o: EditCliOptions): Promise<void> {
  ...
  if (!hasOps(chain)) fail('No operation given. See `graver edit --help`.');
...
async function runDoc(
  ...
  if (isPdf(bytes)) {
    ...
    const tmp = await mkdtemp(join(tmpdir(), 'graver-doc-'));
    try {
    ...
    fail('`doc` renders a PDF, SVG or Office document (docx/xlsx/pptx/…) to images. For a raster image use `graver vectorize` or `graver edit`.');
    return;
  }
...
async function runCenterline(input: string, o: CenterlineCliOptions): Promise<void> {
  ...
  const out = centerlineTrace(source.image, {
    ...
    generator: `Generated by graver v${VERSION}`,
...
program
  ...
  .option('--soffice <path>', 'path to the LibreOffice soffice binary (else auto-detected, or set $GRAVER_SOFFICE)')
...
program
  .command('mcp')
  .description('Start the MCP server (stdio), exposing graver as tools for AI agents/IDEs')
...
program.parseAsync(process.argv).catch((err: unknown) => {
  ...
  if (process.env.GRAVER_DEBUG && err instanceof Error && err.stack) {
