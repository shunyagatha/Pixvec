...
import {
  PRESETS, measureFlatness, refineGateFor, cleanMinArea, REFINE_NOISE_LIMIT,
  type Preset, type Flatness,
} from './vectorize/presets.js';

export { PRESETS, measureFlatness, type Preset } from './vectorize/presets.js';
...
async function runTrace(
  ...
  // `clean`'s despeckle floor is a property of content, not a preset constant
  // — see `cleanMinArea` (vectorize/presets.ts) for the measurement.
  if (opts.preset === 'clean') {
    preset = { ...preset, minArea: cleanMinArea(input.image.width, input.image.height) };
  }
