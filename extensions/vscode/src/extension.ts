...
interface VectorizeOpts { preset?: string; mode?: string; verify?: boolean }
...
async function run(
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: `Vecline: ${label} ${basename(uri.fsPath)}…` },
    async () => {
      try {
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('vecline.vectorize', (arg?: vscode.Uri) =>
      run(arg, 'converting', async (api, input) => {
    ...
    vscode.commands.registerCommand('vecline.lossless', (arg?: vscode.Uri) =>
      run(arg, 'converting losslessly', async (api, input) => {
        const r = await api.vectorize(input, { mode: 'lossless', verify: true });
        return { out: r.svg, ext: '.svg', bytesIn: input.bytes.length, quality: r.quality };
      })),

    vscode.commands.registerCommand('vecline.component', (arg?: vscode.Uri) =>
      run(arg, 'converting to a component', async (api, input, path) => {
